...
export function SlideTypeDropdown({
  postId,
  slideIndex,
  currentType: initialType,
  onUpdate
}: SlideTypeDropdownProps) {
  ...
  const handleTypeChange = async (newType: 'hook' | 'content' | 'cta') => {
    ...
    try {
      const response = await fetch(
        `/api/tiktok/posts/${postId}/slides/${slideIndex}/classification`,
        {
          method: 'PATCH',
          headers: {
      ...
      // Don't trigger full refetch - optimistic update is already applied
    } catch (error) {
