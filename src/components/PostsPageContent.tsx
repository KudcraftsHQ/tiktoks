...
export interface PostsPageContentProps {
  /**
   * Optional profile ID to filter posts by a specific profile
   */
  profileId?: string
  /**
   * Optional profile handle for URL building when filtering by profile
   */
  profileHandle?: string
  /**
   * Custom title element to render in the header
   */
  titleElement?: React.ReactNode
  /**
   * Additional header actions to render alongside the default actions
   */
  additionalHeaderActions?: React.ReactNode
  /**
   * Columns to hide in the posts table
   */
...
export function PostsPageContent({
  ...
  // Build base path for URL updates
  const basePath = profileHandle ? `/profiles/${profileHandle}` : '/'
  ...
  const updateURL = useCallback((page: number, sort: SortingState, dateFilter: DateRange, category: string, search: string, filters: AdvancedFiltersValue) => {
    ...
    // Add advanced filters (only if not filtering by profile)
    if (!profileId) {
  ...
  const fetchPosts = useCallback(async (page: number, limit: number, sort: SortingState, dateFilter: DateRange, category: string, search: string, filters: AdvancedFiltersValue): Promise<void> => {
    ...
    try {
      ...
      if (profileId) {
        params.append('accountIds', profileId)
      } else {
        // Only use advanced filters accountIds if not filtering by profile
        if (filters.accountIds.length > 0) {
  ...
  return (
    <div className="flex h-screen w-full min-w-0">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <PageLayout
          title={titleElement || defaultTitle}
          headerActions={
              <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto items-center">
                {/* Show search in header if custom title element is provided */}
                {titleElement && (
                  <SearchInput
                    value={searchQuery}
                    onChange={handleSearchChange}
